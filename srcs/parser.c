...
/*   Updated: 2021/03/24 15:08:12 by rzukale          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_data(t_png *png)
{
	...
	png->i = 33;
	if (!(png->compressed = (unsigned char *)malloc(sizeof(unsigned char)
		* png->compressed_size)))
		error_output("Memory allocation of compressed data pointer failed\n");
	copy_idat_chunks(png);
}
...
void	decode_png(t_png *png)
{
	png->inflated_size = ((png->width * (png->height * png->bpp + 7)) / 8) +
		png->height;
	if (!(png->inflated = (unsigned char *)malloc(sizeof(unsigned char)
		* png->inflated_size)))
...
{
	if (!(png->source.buf = (unsigned char *)malloc(sizeof(unsigned char)
		* size)))
